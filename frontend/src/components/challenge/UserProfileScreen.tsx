...
const PostItem = ({ item }) => {
  return (
    ...
    >
      <Image source={require("../../../assets/sample/sample8.jpg")} style={styles.image} />
    </TouchableOpacity>
  );
};
...
export default function UserProfileScreen({ route }: any) {
  ...
  const [authorEmail, setAuthorEmail] = useState<string>(route.params.authorEmail);
  ...
  // 사용자 정보 조회
  const getUser = () => {
    axiosInstance.get(`/api/user?email=${authorEmail}`)
  ...
  const getProfileImg = () => {
    axiosInstance.get(`/api/user/image?email=${authorEmail}`)
      .then(function (response) {
  ...
  useEffect(() => {
    axiosInstance.get(`/api/mypage/${tabList[activeTab]}?email=${authorEmail}`)
      .then(function (response) {
